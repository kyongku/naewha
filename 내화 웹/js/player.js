...
const PlayerSpriteAssets = {
  idle: loadImageAsset('assets/sprites/player/player_idle.png'),
  run01: loadImageAsset('assets/sprites/player/player_run_01.png'),
  run02: loadImageAsset('assets/sprites/player/player_run_02.png'),
  jump: loadImageAsset('assets/sprites/player/player_jump.png'),
  dash: loadImageAsset('assets/sprites/player/player_dash.png'),
  axe: loadImageAsset('assets/sprites/weapons/axe.png'),
  fallbackIdle: loadImageAsset('assets/sprites/player-idle.svg'),
  fallbackAttack: loadImageAsset('assets/sprites/player-hit.svg'),
  fallbackDash: loadImageAsset('assets/sprites/player-dash.svg'),
};

const PlayerVfxAssets = {
  axeSlash: loadImageAsset('assets/vfx/axe_slash.png'),
  hitSpark: loadImageAsset('assets/vfx/hit_spark.png'),
  impactRing: loadImageAsset('assets/vfx/impact_ring.png'),
  impactRingGround: loadImageAsset('assets/vfx/impact_ring_ground.png'),
};
...
class ComboSystem {
    ...
    this.axeHits = [
      { startup:2, active:4, recovery:2, damage:45 },
      { startup:2, active:4, recovery:2, damage:45 },
      { startup:2, active:4, recovery:2, damage:45 },
    ];
    // Faster, weaker fallback when the axe is not in hand.
    this.unarmedHits = [
      { startup:1, active:3, recovery:2, damage:22 },
      { startup:1, active:3, recovery:2, damage:22 },
      { startup:1, active:3, recovery:2, damage:22 },
    ];
    this.axeHitCd     = [10, 10, 10];
    this.unarmedHitCd = [10, 10, 10];
    this.TIMEOUT    = 60;
...
class AxeSystem {
  ...
  onAxePickedUp() {
    ...
    this._lastPathFrom = { x: Infinity, y: Infinity };
    this._lastPathTo   = { x: Infinity, y: Infinity };
    if (this.player.comboSys) {
      this.player.comboSys._attackStyle = 'axe';
    }
  }
...
class OxygenSystem {
  ...
  setEnabled(v) {
    this.enabled = !!v;
    if (!this.enabled) {
      if (this._channeling) this.requestChannelStop();
      this._bursting = false;
      this._burstTimer = 0;
      this.stacks = 0;
    }
...
class Player {
  constructor(x, y) {
    ...
    this.baseSpeed    = 200;
      this.JUMP_VEL     = -440;
      this.DJUMP_VEL    = -355;
      this.COYOTE_TIME = 6 * DT;
      this.JUMP_BUFFER_TIME = 6 * DT;
      this.JUMP_CUT_MULTIPLIER = 0.5;
      this.FALL_GRAVITY_MULTIPLIER = 1.4;
      this.FAST_FALL_GRAVITY_MULTIPLIER = 1.8;
    this.MAX_JUMPS    = 2;
    ...
    this.phase        = HP_PHASE.NORMAL;
      ...
      this.invincibleTimer = 0;
        ...
        this._lastOneWayPlatform = null;
        this._lastOneWayPlatformTimer = 0;
    ...
    this.UNARMED_ATCK_LEN = 64;
    this.UNARMED_ATCK_W = 24;
    this.UNARMED_HEAT_GAIN_MULTIPLIER = 0.5;
      this._lastAttackStyle = 'axe';
      this.ATTACK_FACING_LOCK = 8 * DT;
      this._attackFacingLockTimer = 0;
    ...
    this.ATCK_W       = 30;
      ...
      this._runAnimTime = 0;
      this._lastSpriteDrawUsedImage = false;
      }
  ...
  processInput(entities) {
    // 醫뚰겢由? 肄ㅻ낫
      if (Input.isMbJust(0)) {
        if (!this.dashSys.isDashing() && !this.oxygenSys.isChanneling()) {
          ...
          if (Math.abs(d.x) > 0.1) {
            this.facing = Math.sign(d.x);
            this._attackFacingLockTimer = this.ATTACK_FACING_LOCK;
          }
    ...
    if (Input.isJust('KeyW')) {
      ...
      if (Input.isJust('Space')) {
        if (this._isDownHeld() && this._canDropThroughPlatform()) {
          this._startDropThrough();
          return;
        }
        this._jumpBufferTimer = this.JUMP_BUFFER_TIME;
        this._tryImmediateJump();
      }
      if (Input.isJustRel('Space') && this.vy < 0) {
    ...
    _canDropThroughPlatform() {
      const current = this._findDropThroughPlatformUnderfoot();
      if (current) {
        this._lastOneWayPlatform = current;
        this._lastOneWayPlatformTimer = 0.12;
        return true;
      }
      return this._lastOneWayPlatformTimer > 0 && !!this._lastOneWayPlatform;
    }

    _findDropThroughPlatformUnderfoot() {
      if (!this._currentTiles) return null;
      const r = eRect(this);
      const margin = 12;
      const footEps = 8;
      for (const tile of this._currentTiles) {
        if (!tile || !tile.oneWay) continue;
        const overlapsX = r.r > tile.l - margin && r.l < tile.r + margin;
        const footOnTop = r.b >= tile.t - footEps && r.b <= tile.t + footEps;
        if (overlapsX && footOnTop) return tile;
      }
      return null;
    }
  ...
  _tryImmediateJump() {
    const canGroundJump = this.onFloor || this._coyoteTimer > 0;
    if (canGroundJump && this.jumpCount === 0) {
      return this._performGroundJump();
    }
    if (!this.onFloor && this.jumpCount > 0 && this.jumpCount < this.MAX_JUMPS) {
      return this._performAirJump();
    }
    return false;
  }

  _tryBufferedGroundJump() {
    if (this._jumpBufferTimer <= 0) return false;
    const canGroundJump = this.onFloor || this._coyoteTimer > 0;
    if (!canGroundJump || this.jumpCount !== 0) return false;
    return this._performGroundJump();
  }
  ...
  physicsUpdate(dt, tiles, entities) {
      ...
      this._runAnimTime += dt;
        ...
        if (this._lastOneWayPlatformTimer > 0) {
          this._lastOneWayPlatformTimer = Math.max(this._lastOneWayPlatformTimer - dt, 0);
          if (this._lastOneWayPlatformTimer <= 0) this._lastOneWayPlatform = null;
        }
      if (this._jumpBufferTimer > 0) {
        this._jumpBufferTimer = Math.max(this._jumpBufferTimer - dt, 0);
      }
      if (this.onFloor) {
        this._coyoteTimer = this.COYOTE_TIME;
      } else if (this._coyoteTimer > 0) {
        this._coyoteTimer = Math.max(this._coyoteTimer - dt, 0);
      }
        ...
        if (this._attackFacingLockTimer > 0) {
          this._attackFacingLockTimer = Math.max(this._attackFacingLockTimer - dt, 0);
        }
    ...
    if (axeDashActiveAtStart) {
      ...
      // 以묐젰
      if (!this.onFloor) {
        const gravityMul = this.vy > 0
          ? (this._isDownHeld() ? this.FAST_FALL_GRAVITY_MULTIPLIER : this.FALL_GRAVITY_MULTIPLIER)
          : 1;
        this.vy = Math.min(this.vy + GRAVITY * gravityMul * dt, MAX_FALL);
      }
    ...
    if (this.axeSys.isDashing()) {
      ...
      this.vx = 0;
        ...
        if (dir !== 0 && (!this.comboSys.isAttacking() || this._attackFacingLockTimer <= 0))
    ...
    if (this.axeSys.isDashing()) {
      ...
      this.onWall  = result.onWall;
        if (result.onFloor) {
          this.jumpCount = 0;
          this._coyoteTimer = this.COYOTE_TIME;
          const oneWay = this._findDropThroughPlatformUnderfoot();
          if (oneWay) {
            this._lastOneWayPlatform = oneWay;
            this._lastOneWayPlatformTimer = 0.12;
          }
        }
    }

    this._tryBufferedGroundJump();
  ...
  _onAttackActive(idx, damage, attackStyle = 'axe') {
    ...
    const attackAngle = angle(this._atkDir.x, this._atkDir.y);
    const SPRITE_SLASH_ROT_OFFSET = Math.PI;
    const swingProfiles = isUnarmed
      ? [
          { offset: 0.28, forward: 0.16, radius: 30, a0: -0.82, a1: 0.38, alpha: 0.6, width: 3.5, sparkLen: 12 },
          { offset: 0.34, forward: 0.2, radius: 32, a0: -0.98, a1: 0.54, alpha: 0.7, width: 4.25, sparkLen: 14 },
          { offset: 0.38, forward: 0.24, radius: 35, a0: -1.08, a1: 0.7, alpha: 0.78, width: 4.8, sparkLen: 15 },
        ]
      : [
          { offset: 0.32, forward: 0.18, radius: 38, a0: -0.9, a1: 0.42, alpha: 0.74, width: 5.5, sparkLen: 17 },
          { offset: 0.39, forward: 0.24, radius: 42, a0: -1.08, a1: 0.58, alpha: 0.82, width: 6.4, sparkLen: 19 },
          { offset: 0.46, forward: 0.3, radius: 47, a0: -1.2, a1: 0.82, alpha: 0.9, width: 7.2, sparkLen: 21 },
        ];
      const swing = swingProfiles[idx] || swingProfiles[swingProfiles.length - 1];
      const slashOffset = isUnarmed ? 28 : 34;
      const slashX = this.x + this._atkDir.x * slashOffset;
      const slashY = (this.y - 32) + this._atkDir.y * slashOffset;
      const useSpriteSlash =
        PlayerVfxAssets.axeSlash.loaded &&
        !PlayerVfxAssets.axeSlash.failed;
          if (useSpriteSlash) {
            addVfx({
              type: 'sprite',
              asset: PlayerVfxAssets.axeSlash,
              x: slashX,
              y: slashY,
              w: isUnarmed ? 96 : 168,
              h: isUnarmed ? 44 : 72,
              rot: attackAngle + SPRITE_SLASH_ROT_OFFSET,
              flipX: false,
              anchorX: 0.5,
              anchorY: 0.5,
              dur: 0.14,
              alpha: isUnarmed ? 0.72 : 0.84,
              scaleRate: 0.04,
              blend: 'lighter',
              t: 0,
            });
        ...
        addVfx({
          ...
          a1: swing.a1,
          rot: attackAngle,
          color: isUnarmed ? '#ffd08a' : '#ff8248',
          coreColor: isUnarmed ? '#fff2cc' : '#ffe0b8',
          lineWidth: swing.width,
          dur: 0.14,
          alpha: swing.alpha,
          t: 0,
        });
    ...
    for (const e of hits) {
      if (e.isDead) continue;
        ...
        const hitX = clamp(cx, e.x - e.hw, e.x + e.hw);
        const hitY = clamp(cy, e.y - e.height, e.y);
        if (PlayerVfxAssets.hitSpark.loaded && !PlayerVfxAssets.hitSpark.failed) {
          addVfx({
            type: 'sprite',
            asset: PlayerVfxAssets.hitSpark,
            x: hitX,
            y: hitY,
            w: 40,
            h: 40,
            rot: attackAngle + (Math.random() - 0.5) * 0.6,
            anchorX: 0.5,
            anchorY: 0.5,
            dur: 0.12,
            alpha: 0.95,
          ...
          addVfx({
            ...
            len: swing.sparkLen,
          rays: isUnarmed ? 5 : 7,
          rot: attackAngle + (Math.random() - 0.5) * 0.4,
          color: isUnarmed ? '#fff0b8' : '#ffd2a8',
        lineWidth: isUnarmed ? 2.2 : 2.8,
          dur: 0.12,
          alpha: 0.9,
          t: 0,
        });
        }
        if (PlayerVfxAssets.impactRing.loaded && !PlayerVfxAssets.impactRing.failed) {
          addVfx({
            type: 'sprite',
            asset: PlayerVfxAssets.impactRing,
            x: hitX,
            y: hitY,
            w: isUnarmed ? 44 : 56,
            h: isUnarmed ? 44 : 56,
          ...
          addVfx({
            ...
            y: hitY,
            r: isUnarmed ? 10 : 13,
            color: isUnarmed ? '#ffe4a4' : '#ffb07a',
            lineWidth: isUnarmed ? 2 : 2.6,
            expand: 0.28,
            dur: 0.12,
            alpha: 0.65,
            t: 0,
  ...
  takeDamage(amount) {
    ...
    if (this.smokeSys && this.smokeSys.tryFullGuard()) return;
  ...
  _resolveSpriteAsset(primary, fallback) {
    if (primary && primary.loaded && !primary.failed) return primary;
  ...
  _getCurrentSpriteAsset() {
    if (this.dashSys.isDashing()) {
      return {
        asset: this._resolveSpriteAsset(PlayerSpriteAssets.dash, PlayerSpriteAssets.fallbackDash),
        targetHeight: 72,
        anchorX: 0.56,
      };
    }
    const airborneVisual = !this.onFloor && Math.abs(this.vy) > 18;
    if (airborneVisual) {
      return {
        asset: this._resolveSpriteAsset(PlayerSpriteAssets.jump, PlayerSpriteAssets.idle),
        targetHeight: 72,
        anchorX: 0.5,
      };
    }
    const horizontalHeld = this._isHorizontalHeld();
    if (horizontalHeld) {
      return {
        asset: this._resolveSpriteAsset(PlayerSpriteAssets.run01, PlayerSpriteAssets.fallbackIdle),
        targetHeight: 72,
        anchorX: 0.54,
      };
    }
    return {
      asset: this._resolveSpriteAsset(PlayerSpriteAssets.idle, PlayerSpriteAssets.fallbackIdle),
      targetHeight: 72,
      anchorX: 0.5,
    };
  }
  ...
  _drawSpriteBody(ctx, sx, sy, style) {
    const sprite = this._getCurrentSpriteAsset();
    const asset = sprite?.asset;
    if (!asset || !asset.loaded || asset.failed) return false;
    const targetH = sprite?.targetHeight ?? this._spriteH;
    const sourceW = asset.width || this._spriteW;
    const sourceH = asset.height || this._spriteH;
    const drawH = targetH;
    const drawW = sourceH > 0 ? (sourceW / sourceH) * drawH : this._spriteW;
    const anchorX = sprite?.anchorX ?? 0.5;
    const anchorY = 1;

    const drawSX = Math.round(sx);
    const drawSY = Math.round(sy);
    ctx.save();
    ctx.translate(drawSX, drawSY);
    ctx.scale(this.facing >= 0 ? 1 : -1, 1);
    ctx.drawImage(asset.img, -drawW * anchorX, -drawH * anchorY, drawW, drawH);
    ctx.restore();
    this._lastSpriteDrawUsedImage = true;
    return true;
  }
  ...
  draw(ctx) {
    ...
    if (this.oxygenSys.isChanneling()) {
      ...
      this._drawFormOverlay(ctx, sx, sy, formStyle);
        ...
        // ?쇨뎬 諛⑺뼢 ?쒖떆
      if (!this._lastSpriteDrawUsedImage) {
        ctx.fillStyle = '#fff';
    ...
    // 怨듦꺽 踰붿쐞 ?쒖떆
    if (window.NAEHWA_DEBUG_MODE && this._atkFlash > 0 && this.comboSys.isAttacking()) {
      const attackAngle = angle(this._atkDir.x, this._atkDir.y);
      const isUnarmed = this._lastAttackStyle === 'unarmed';
      const arcRadius = isUnarmed ? 26 : 34;
      ctx.save();
      ctx.globalAlpha = 0.24;
      ctx.translate(sx, sy - 32);
      ctx.rotate(attackAngle);
      ctx.strokeStyle = isUnarmed ? '#ffe0a6' : '#ffb078';
      ctx.lineWidth = isUnarmed ? 4 : 5;
      ctx.beginPath();
      ctx.arc(0, 0, arcRadius, -0.72, 0.72);
      ctx.stroke();
      ctx.restore();
    }
  }
