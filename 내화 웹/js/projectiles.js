...
// ?? ?꾨겮 ?ъ궗泥?????????????????????????????????????????
const AxeSpriteAsset = typeof loadImageAsset === 'function'
  ? loadImageAsset('assets/sprites/weapons/axe.png')
  : null;
const AXE_PROJECTILE_ROT_OFFSET = 0;
...
class Axe {
  ...
  draw(ctx) {
    ...
    const spriteReady = AxeSpriteAsset && AxeSpriteAsset.loaded && !AxeSpriteAsset.failed;
    let drawRot = this.rotation;
    if (this.state === 'flying') {
      drawRot = Math.atan2(this.vy, this.vx) + AXE_PROJECTILE_ROT_OFFSET;
    }
    ctx.save();
    ctx.translate(s.x, s.y);
    ctx.rotate(drawRot);
    if (spriteReady) {
      ctx.drawImage(AxeSpriteAsset.img, -22, -22, 44, 44);
    } else {
