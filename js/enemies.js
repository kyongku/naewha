...
class Enemy {
  ...
  takeDamage(amount, sourceType = 'field', sourceAttacker = null) {
    ...
    if (sourceType === 'melee' || sourceType === 'melee_unarmed') {
      const attackerX = sourceAttacker?.x ?? this._player?.x ?? (this.x - 1);
      const pushDir = Math.sign(this.x - attackerX) || 1;
      this.vx = pushDir * Math.min(140, this.knockbackDist * 2.5);
      return amount;
    }
    this._enterStun();
    return amount;
  ...
  _getPlayerContext() {
    if (!this._player) {
      return {
        ...
        verticalDistance: 0,
        verticalDistanceAbs: 0,
        playerAbove: false,
        playerOnPlatformLikely: false,
      };
    }
    const selfCenterY = this.y - this.height / 2;
    const playerCenterY = this._player.y - this._player.height / 2;
    const verticalDistance = playerCenterY - selfCenterY;
    const verticalDistanceAbs = Math.abs(verticalDistance);
    const horizontalDistance = Math.abs(this._player.x - this.x);
    const playerAbove = verticalDistance < -52;
    return {
      horizontalDistance,
      verticalDistance,
      verticalDistanceAbs,
      playerAbove,
      playerOnPlatformLikely: playerAbove && verticalDistanceAbs > 72,
    };
  }
  ...
  draw(ctx) {
    ...
    if (this.isDead) {
      ...
      ctx.fillStyle = this._hitFlash > 0
        ? '#fff4dc'
        : (this.state === 'stun' ? '#fff' : this.color);
...
class Ember extends Enemy {
  ...
  _onStateEntered(s) {
    ...
    if (s === 'attack') {
      ...
      const ctx = this._getPlayerContext();
      if (this._player) {
        const d = norm(this._player.x - this.x, this._player.y - this.y);
        this._dashDir = ctx.playerOnPlatformLikely
          ? { x: Math.sign(d.x) || this._dirToPlayer(), y: 0 }
          : d;
      } else {
  ...
  _processState(dt) {
    switch(this.state) {
      ...
      case 'chase':
        {
          const ctx = this._getPlayerContext();
          if (ctx.playerOnPlatformLikely) {
            this.vx = this._dirToPlayer() * this.DASH_SPEED * 0.24;
            if (ctx.horizontalDistance < 28) this.vx = 0;
            break;
          }
          this.vx = this._dirToPlayer() * this.DASH_SPEED * 0.18;
...
class Golem extends Enemy {
  constructor(x, y) {
    ...
    this.SLAM_RANGE   = 140;
    this.SLAM_W       = 150; this.SLAM_H = 64;
    this.SLAM_DMG     = 100;
    this.SLAM_STARTUP = 60; this.SLAM_RECOVERY = 42;
  ...
  _handleChase() {
    ...
    const horizontalGap = this._player ? Math.abs(this._player.x - this.x) : 0;
    ...
    if (verticalGap > 72) {
      if (horizontalGap < 36) {
        this.vx = (this.tick % 80 < 40 ? -1 : 1) * this.CHASE_SPEED * 0.85;
      }
      return;
    }
  ...
  _onStateEntered(s) {
    if (s === 'attack') {
      ...
      this._attackDir = this._dirToPlayer() || this._patrolDir || 1;
    }
  }
  ...
  _doSlam() {
    ...
    {
    const rect = this._getSlamRect();
    const hbL = rect.x, hbT = rect.y;
    const hbR = hbL + rect.w, hbB = hbT + rect.h;
    const pr = eRect(this._player);
    if (pr.l < hbR && pr.r > hbL && pr.t < hbB && pr.b > hbT)
      this._player.takeDamage(this.SLAM_DMG);
    addVfx({ type:'rect', x: rect.cx, y: rect.cy,
             w: rect.w, h: rect.h, rot:0, color:'#c0a060', dur:0.2, alpha:0.5, t:0 });
    return;
    }
  ...
  _getSlamRect() {
    const dir = this._attackDir || this._dirToPlayer() || 1;
    const cx = this.x + dir * (this.SLAM_W * 0.5 + this.hw * 0.35);
  ...
  draw(ctx) {
    ...
    if (!this.isDead && this.state === 'attack') {
      if (this._attackType === 'slam' && !this._inRecovery) {
        const rect = this._getSlamRect();
        const s = Camera.toScreen(rect.x, rect.y);
        const progress = clamp(this.tick / this.SLAM_STARTUP, 0, 1);
        ctx.save();
        ctx.strokeStyle = 'rgba(255, 186, 82, 0.85)';
        ctx.lineWidth = 2;
        ctx.strokeRect(s.x, s.y, rect.w, rect.h);
        ctx.fillStyle = 'rgba(255, 156, 54, 0.18)';
        ctx.fillRect(s.x, s.y, rect.w * progress, rect.h);
        ctx.fillStyle = 'rgba(255, 230, 150, 0.32)';
        ctx.fillRect(s.x, s.y, rect.w * progress, 4);
        ctx.restore();
        return;
      }
      // 공격 준비 표시
