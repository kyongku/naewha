...
const TEST_SHORTCUTS_ENABLED = true;
...
const GameManager = {
  ...
  applyBossReward(bossId) {
    ...
    if (!this.clearedBossRewards.includes(bossId))
      this.clearedBossRewards.push(bossId);
  ...
  syncProgressUnlocks() {
    const cleared = new Set(this.clearedBossRewards || []);
    this.hasOxygen = cleared.has('boss1');
    this.hasDash = this.hasOxygen;
    this.hasHeat = cleared.has('boss2');
    this.hasAxeUpgrade = this.hasHeat;
    this.hasSmoke = cleared.has('boss3');
    this.hasShield = this.hasSmoke;
    if (TEST_UNLOCK_ALL) {
      this.hasOxygen = true;
      this.hasDash = true;
      this.hasHeat = true;
  ...
  applyUnlocks(player) {
      ...
      player.oxygenSys.stacks = 0;
      player.oxygenSys.shieldUnlocked = false;
    ...
    if (TEST_UNLOCK_ALL) this._refillTestResources(player);
    this.applyRecoveryModifiers(player);
  },
  _refillTestResources(player) {
    if (!player) return;
    player.hp = player.MAX_HP;
    if (player.oxygenSys && player.oxygenSys.enabled)
      player.oxygenSys.stacks = player.oxygenSys.MAX_STACKS;
    if (player.heatSys && player.heatSys.enabled)
      player.heatSys.value = player.heatSys.maxValue;
    if (player.smokeSys && player.smokeSys.enabled)
      player.smokeSys.value = player.smokeSys.maxValue;
  },
  onPlayerDeath() {
    this.deathCount++;
...
class SceneState {
  ...
  _initRooms() {
    ...
    [new Ember(420, FLOOR_Y), new MirrorShard(900, FLOOR_Y), new Ember(1520, FLOOR_Y), new Golem(2140, FLOOR_Y)]
  ...
  _debugGoToBossRoom(idx, label) {
    GameManager.syncProgressUnlocks();
    GameManager.applyUnlocks(this.player);
    this._debugGoToRoom(idx, 'right');
    Projectiles.clear();
    GameManager.applyUnlocks(this.player);
    this._syncBossHUD();
    GameManager._showDebugToast(`Moved to ${label}`);
  }
  ...
  processInput() {
      ...
      if (TEST_SHORTCUTS_ENABLED) {
        if (Input.isJust('Digit0')) {
          GameManager.syncProgressUnlocks();
          GameManager.applyUnlocks(this.player);
          GameManager._showDebugToast('Test unlocks/resources refilled');
          return;
        }
        if (Input.isJust('Digit9')) { this._debugGoToBossRoom(1, 'Boss1'); return; }
        if (Input.isJust('Digit8')) { this._debugGoToBossRoom(3, 'Boss2'); return; }
        if (Input.isJust('Digit7')) { this._debugGoToBossRoom(5, 'Boss3'); return; }
        if (Input.isJust('Digit6')) { this._debugGoToBossRoom(9, 'Final Boss'); return; }
      }
      if (DEBUG_MODE) {
        if (Input.isJust('F1')) {
          GameManager.unlockOxygen();
...
const Game = {
  ...
  resetFull() {
    ...
    GameManager.clearedRooms  = [];
    GameManager.clearedBossRewards = [];
